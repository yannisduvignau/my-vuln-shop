...
import { useLocalStorageToken } from '../hooks/useLocalStorageToken.ts';
import { Link } from 'react-router-dom';
import { Search, ShoppingBag, User, Menu, X, UserCheck } from 'lucide-react';

const Navbar = () => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  // On récupère le token du localStorage en temps réel
  const accessToken = useLocalStorageToken("accessToken");
  const userId = localStorage.getItem("userId"); // ou tu peux aussi faire un hook similaire
          ...
          <div className="hidden md:flex items-center space-x-5">
            ...
            </div>
            {
              !isConnected ? (
                ...
                <Link to={`/users/${localStorage.getItem("userId")}`} className="text-green-500 hover:text-green-900">
                  <UserCheck size={24} />
                </Link>
              )
            }
